...
private:
  ...
  ros::Time time_stamp_; // main timesynched time based on image
  ros::Time image_time_stamp_;
  ros::Time lidar_time_stamp_;
  ros::Time lidar_first_time_stamp_;
...
void ImageCloudDataExport::ImageCallback(const sensor_msgs::CompressedImageConstPtr &in_image_msg)
{
  ...
  // } // ver 2
    ...
    ss << image_time_stamp_.sec << "." << std::setw(9) << std::setfill('0') << image_time_stamp_.nsec;
    time_stamp_ss = ss.str();
    ROS_INFO("[ImageCloudDataExport] Image Frame Time: %s", time_stamp_ss.c_str());

    // 3) save into txt file
    std::string timestamp_path = path_timestamp_str_ + "timestamp_image.txt";
...
{
  // saving image's timestamp for the sake of LiDAR's OSC time can be remapped with this (2023.11.27)
...
void ImageCloudDataExport::LidarCloudCallback(const sensor_msgs::PointCloud2ConstPtr &in_sensor_cloud)
{
  ...
  if(!sync_topics_)
  {
    ...
    std::stringstream ss;
    std::string time_stamp_ss;
    ss << ros_time.sec << "." << std::setw(9) << std::setfill('0') << ros_time.nsec;
    time_stamp_ss = ss.str();
    ROS_INFO("[ImageCloudDataExport] LiDAR Frame Time: %s", time_stamp_ss.c_str());

    // 3) save into txt file
    std::string timestamp_path = path_timestamp_str_ + "timestamp_pcd.txt";
    std::ofstream outputFile(timestamp_path, std::ios::app);
    // outfile.open(timestamp_path);
    if (!outputFile.is_open()) {
      std::cerr << "Error: Unable to open the timestamp file." << std::endl;
  ...
  // -----------------------------------------------------
  pcl::io::savePCDFileASCII(file_path + ".pcd", *cloud_ptr); // actual point cloud save
}
...
// main function
{
  ...
  private_node_handle.param<std::string>("bag_file_path", bag_file_path, "");
  ROS_INFO("[ImageCloudDataExport] bag_file_path: %s", bag_file_path.c_str());

  // ROSBAG metadata reader
  if (bag_file_path != "")
  {
    // bag file reader test
    // https://stackoverflow.com/questions/74629077/loading-rosbag-duration-using-cpp
    rosbag::Bag bag;
    ...
    bag_start_time_ = bag_begin_time; // member property save
    
    std::cout << "ROS bag begin time: " << bag_begin_time << std::endl;
    std::cout << "orignal: " << (bag_begin_time).toSec() << "(s)" << std::endl;
    ...
    for (rosbag::MessageInstance const m : rosbag::View(bag))
    {
        ...
        if (m.getTopic() == "/ouster/lidar_packets")
        {
          ros::Time first_time = m.getTime();
          ROS_INFO("Message time: %f", first_time.toSec());
          lidar_first_time_stamp_ = first_time;
          break;
        }
    }  
    std::cout << "found first lidar time: " << lidar_first_time_stamp_ << std::endl;
    bag.close();
  }
